...
#'   
#' @param start_col Integer; the starting column index for the first survey item. 
...
score_rand36sf <- function(data, start_col = 3) {
  offset <- start_col - 1
  ...
  recoded_data |> mutate(
    `Physical functioning` = rowMeans(recoded_data[,c(3:12)+offset], na.rm = TRUE),
    `Role limitations due to physical health` = rowMeans(recoded_data[,c(13:16)+offset], na.rm = TRUE),
    `Role limitations due to emotional problems` = rowMeans(recoded_data[,c(17:19)+offset], na.rm = TRUE),
    `Energy/fatigue` = rowMeans(recoded_data[,c(23,27,29,31)+offset], na.rm = TRUE),
    `Emotional well-being` = rowMeans(recoded_data[,c(24,25,26,28,30)+offset], na.rm = TRUE),
    `Social functioning` = rowMeans(recoded_data[,c(20,32)+offset], na.rm = TRUE),
    `Pain` = rowMeans(recoded_data[,c(21,22)+offset], na.rm = TRUE),
    `General health` = rowMeans(recoded_data[,c(1,33:36)+offset], na.rm = TRUE))
}
